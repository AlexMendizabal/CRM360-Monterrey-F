...
export class ComercialLoteRutaComponent implements OnInit {
        ...
        (response: any) => {
          if (response && response.hasOwnProperty('data')) {
            ...
            this.mapas.forEach((mapa) => {
              ...
              if (mapa.color === 1) {
                mapa.markers.icon =
                  'https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|FF0000'; // Rojo
              } else if (mapa.color === 2) {
                mapa.markers.icon =
                  'https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|FC9F3A'; // Naranja
              } else if (mapa.color === 3) {
                mapa.markers.icon =
                  'https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|FFFF00'; // Amarillo
              } else if (mapa.color === 4) {
                mapa.markers.icon =
                  'https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|FFFFFF'; // Blanco
              } else {
                mapa.markers.icon =
                  'https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|00FF00'; // Verde
              }
            });
          } else {
            this.mapas = null;
  ...
  agregarClienteTemporal(mapa: any) {
    const nuevoCliente = {
      ...
      dataInicia: mapa.data_inicia,
      dataConclusao: mapa.data_conclusao,
    ...
    /*  mapa.markers['icon'] = marker; */
    this.selectVendedorDefault(nuevoCliente, mapa);
  }

  selectVendedorDefault(cliente: any, mapa: any) {
    const vendedorEncontrado = this.filteredVendedores.find(
      (vendedor) => vendedor.ID === mapa.ID_VENDEDOR
    );
  ...
  eliminarClienteTemporal(item: any) {
    ...
    let registro = 0;
    registro = this.mapas.findIndex(
      (mapa) => mapa.CODIGO_CLIENTE === item.codigoCliente
    );

    if (registro !== -1) {
      this.atividades.splice(index, 1);
      if (this.mapas[registro].color === 1) {
        this.mapas[registro].markers.icon =
          'https://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|FF0000'; // Rojo
