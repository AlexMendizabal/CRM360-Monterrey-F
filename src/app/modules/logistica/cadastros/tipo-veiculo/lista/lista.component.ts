...
export class LogisticaTipoVeiculoListaComponent implements OnInit, OnDestroy {
  ...
  onActivatedRoute() {
    this.$activatedRouteSubscription = this.activatedRoute.queryParams.subscribe(
      (response) => {
        ...
        this.getTipoVeiculos(this.getParams());
      }
    );
  }
      ...
      .subscribe(
        (response) => {
          if (response.status === 200) {
            this.tipoVeiculos = response.body['data'];
  ...
  changeType(tipoVeiculo: ILogisticaTipoVeiculo) {
    const stat = tipoVeiculo.IN_STAT == '1' ? '0' : '1';
    ...
    this.confirmChange(stat)
      ...
      .pipe(
        ...
        switchMap((result) => {
          ...
          return this.tipoVeiculoService.postTipoVeiculo(tipoVeiculo);
        }),
        finalize(() => {
          this.loadingNavBar = false;
        })
  ...
  openRegister(tipoVeiculo: ILogisticaTipoVeiculo) {
