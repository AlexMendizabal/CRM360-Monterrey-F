...
@Component({
  selector: 'app-admin-perfiles-lista',
  standalone: true,
  imports: [
...
export class AdminPerfilesListaComponent implements OnInit {
  ...
  loading = true;
  data: any[] = [];
  displayedColumns: string[] = ['id', 'nome', 'sigla', 'situacao', 'acciones'];
  ...
  getData() {
    ...
    this.service.getPerfiles(params)
      ...
      .subscribe({
        next: (response: any) => {
          if (response.status === 200 && response.body?.data) {
            ...
            this.data = [];
            this.totalItems = 0;
            this.snackBar.open('No se encontraron perfiles.', 'Cerrar', { duration: 3000 });
          }
        },
        error: () => {
          this.data = [];
