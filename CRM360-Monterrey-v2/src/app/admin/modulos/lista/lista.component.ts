...
@Component({
  selector: 'app-admin-modulos-lista',
  standalone: true,
  imports: [
...
export class AdminModulosListaComponent implements OnInit {
  ...
  loading = true;
  data: any[] = [];
  displayedColumns: string[] = ['id', 'nome', 'rota', 'icone', 'situacao', 'acciones'];
  ...
  getData() {
    ...
    this.service.getModulos(params)
      ...
      .subscribe({
        next: (response: any) => {
          if (response.status === 200 && response.body?.data) {
            ...
            this.data = [];
            this.totalItems = 0;
            this.snackBar.open('No se encontraron módulos.', 'Cerrar', { duration: 3000 });
          }
        },
        error: () => {
          this.data = [];
